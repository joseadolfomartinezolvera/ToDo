...
class _CategoriesScreenState extends State<CategoriesScreen> {
  ...
  _showFormInDialog(BuildContext context) {
    ...
    return showDialog(
      //retorna un dialog
        ...
        //Que se pueda salir al dar tap fuera de su area
        builder: (param) {
            ...
            actions: [
              TextButton(
                ...
                onPressed: () async {
                  //Al presionar
                  _category.name =
                      _categoryName.text; //asignale al modelo lo del form
                  _category.description = _categoryDescription.text;
                  var result = await _categoryService.saveCategory(
                      _category); //Usando el servicio de category para persistencia
                  if (result > 0) {
                    Navigator.pop(context); //Sal del cuadro de dialogo.
                    _showSnackBar(
                        "Saved successful!"); //Muestra el mensaje en la barra de snack
                    getAllCategories();
                  }
              ...
              TextButton(
                ...
                onPressed: () async {
                  ...
                  var result = await _categoryService
                      .updateCategory(_category); //usando el service
                  if (result > 0) {
  ...
  Widget build(BuildContext context) {
    // Método para construir los elementos del estado del Widget
    return Scaffold(
      key: scaffoldMessengerKey,
      // El key es para permitir los mensajes en la barra snack
      appBar: AppBar(
        title: Text("Categories"), // Barra de aplicación
        leading: ElevatedButton(
          child: Icon(Icons.arrow_back, color: Colors.white),
          //Botón de la barra
          onPressed: () {
            //Al ser presionado ira a la pantalla de inicio
      ...
      body: ListView.builder(
        // El ListView mostrar todas las categorías registradas
